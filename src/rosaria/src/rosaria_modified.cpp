...
#include <voice_interface/utilsfunctions.h>
#include <sstream>
#include <iostream>
...
class RosAriaNode
{
  ...
  public:
    ...
    void grip_command_cb(const std_msgs::StringConstPtr &);
    void direct_motion_cb(const std_msgs::StringConstPtr &);
  ...
  protected:
    ...
    ros::Subscriber gripcmd_sub;
    ros::Subscriber direct_motion_sub;
...
int RosAriaNode::Setup()
{
  ...
  // connection to gripper
  gripcmd_sub = n.subscribe("gripper_command",1,(boost::function <void(const std_msgs::StringConstPtr&)>) boost::bind(&RosAriaNode::grip_command_cb, this, _1));
  // end connection to gripper
  ...
  direct_motion_sub = n.subscribe("direct_motion_command",1,(boost::function <void(const std_msgs::StringConstPtr&)>) boost::bind(&RosAriaNode::direct_motion_cb, this, _1));
...
void RosAriaNode::cmdvel_watchdog(const ros::TimerEvent& event)
{
  ...
  if ((ros::Time::now() - veltime > ros::Duration(0.6)) && !(dirCommBool))
  {
    robot->lock();
    robot->setVel(0.0);
    if(robot->hasLatVel())
...
void RosAriaNode::grip_command_cb(const std_msgs::StringConstPtr &msg)
{
    if (!(strcmp(msg->data.c_str(),"up")))
        gripper->liftUp();
    else if (!(strcmp(msg->data.c_str(),"down")))
        gripper->liftDown();
    else if (!(strcmp(msg->data.c_str(),"stop")))
        gripper->gripperHalt();
    else if (!(strcmp(msg->data.c_str(),"open")))
        gripper->gripOpen();	
    else if (!(strcmp(msg->data.c_str(),"close")))
        gripper->gripClose();
    else
        ROS_INFO("Ya ahbal, mo4 3arf t compare strings?!");
}

void RosAriaNode::direct_motion_cb(const std_msgs::StringConstPtr &msg)
{
	dirCommBool=true;
    	unsigned int wordCount = countWords(msg->data);
    	vector<string> command = splitStringToWords(msg->data);
	if (!(strcmp(command[0].c_str(),"move")))
	{
		ROS_INFO("Move requested");
		if (!(strcmp(command[1].c_str(),"forward")))
		{
			int distance = stoi(command[2]);
			veltime = ros::Time::now();
			robot->lock();
    			robot->move((double)distance);
			robot->unlock();
		}
		if (!(strcmp(command[1].c_str(),"backward")))
		{
			int distance = stoi(command[2]);
			robot->lock();
    			robot->move(-(double)distance);
	...
	if (!(strcmp(command[0].c_str(),"turn")))
	{
		if (!(strcmp(command[1].c_str(),"left")))
		{
			int angle = stoi(command[2]);
			veltime = ros::Time::now();
			robot->lock();
    			robot->setDeltaHeading((double)angle);
			robot->unlock();
		}
		if (!(strcmp(command[1].c_str(),"right")))
		{
			int angle = stoi(command[2]);
			robot->lock();
    			robot->setDeltaHeading(-(double)angle);
